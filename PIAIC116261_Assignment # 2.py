...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = (arr.transpose() - brr).transpose() 
    return subt
    """
     Expected Output:
               array([[2 2 2]
...
def function11():
    ...
    arr = np.array([0, 1, 2, 1, 4, 1, 6, 1, 8, 1])
    ans = np.copy(arr)
    ans[ans%2==1] *= -1 
    return ans
    """
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.hstack((arr[0],arr[0],arr[0],arr[1],arr[1],arr[1],arr[2],arr[2],arr[2],arr[:3],arr[:3],arr[:3]))
    return ans
    """
     Expected Output:
...
def function18():
    ...
    x = np.count_nonzero(students <= 99)
    return x
    #Expected output: 3
...
def function20():
    ...
    def xyz(x):
        return x*2+3-2
    return np.array([xyz(xi) for xi in x])
